...
def dialog_tag(request, dialog_pk):
    if request.method == 'POST':
        ...
        if tag_name:
            dialog = get_object_or_404(Dialog, pk=dialog_pk)
            print "dialog.tags.through.objects.filter(tag__name=tag_name.lower()).exists()", dialog.tags.through.objects.filter(dialog=dialog, tag__name=tag_name.lower()).exists()
