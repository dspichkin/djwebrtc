...
urlpatterns = [
    ...
    url(r'^(?P<key>\w+)/(?P<token>\w+)/id$', set_streaming, name="set_streaming"),
    url(r'^(?P<key>\w+)/(?P<token>\w+)/candidate$', handle, name="handle"),
    url(r'^(?P<key>\w+)/(?P<token>\w+)/offer$', handle, name="handle"),
]
